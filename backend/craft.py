...
try:
    ...
    crafts_links_collection = db['crafts_links']  # New collection for storing craft links
...
def get_craft_ideas():
    ...
    try:
        ...
        # 2. Check if craft links already exist in MongoDB for this item
        existing_crafts = crafts_links_collection.find_one({"item_name": item_name})
        if existing_crafts:
            print(f"✅ Found existing craft links for '{item_name}' in MongoDB")
            return jsonify(existing_crafts['craft_ideas'])

        # 3. Check if this item is in the top 5 recent detections
        recent_detections = list(history_collection.find(
            {}, 
            {"detected_items.name": 1, "timestamp": 1}
        ).sort("timestamp", -1).limit(5))
        
        # Extract item names from recent detections
        recent_item_names = []
        for detection in recent_detections:
            if detection.get('detected_items'):
                for item in detection['detected_items']:
                    recent_item_names.append(item['name'])
        
        # If current item is not in top 5, return error
        if item_name not in recent_item_names:
            return jsonify({"error": f"'{item_name}' is not in the top 5 recent detections. Please scan it again to generate craft ideas."}), 400

        # 4. Use youtube_client to find relevant videos
        ...
        # 5. Prepare the search results for the Gemini prompt
        search_context = ""
        if youtube_results:
        ...
        if 'candidates' in result and result['candidates']:
            ...
            # 8. Post-process craft_ideas to ensure videoId and thumbnailUrl are correctly assigned
            ...
            # 9. Store the craft ideas in MongoDB for future use
            craft_document = {
                "item_name": item_name,
                "craft_ideas": craft_ideas,
                "youtube_results": youtube_results,  # Store original YouTube results for reference
                "created_at": latest_record.get('timestamp'),  # Use the detection timestamp
                "updated_at": latest_record.get('timestamp')
            }
            
            # Use upsert to avoid duplicates
            crafts_links_collection.update_one(
                {"item_name": item_name},
                {"$set": craft_document},
                upsert=True
            )
            print(f"✅ Stored craft links for '{item_name}' in MongoDB")
